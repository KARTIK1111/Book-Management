...
const createBook = async function (req, res) {
  try {
    ...
    if(!file) return res.status(400).send({status:false,message:"bookcover is required"})
    data['bookCover']=await uploadFile(file)
    ...
    if (!stringVerify(title)) {
      return res
        .status(400)
        .send({ status: false, message: "Title should be of type String." });
    }
    ...
    if (checkTitle) {
      return res
        ...
        .send({
          status: false,
          message: "Title already used, Please provide another title.",
        });
    }
    ...
    if (!stringVerify(excerpt)) {
      return res
        .status(400)
        .send({ status: false, message: "Excerpt should be of type String." });
    }
    ...
    if (!stringVerify(userId)) {
      return res
        .status(400)
        .send({ status: false, message: "UserId should be of type String." });
    }
    ...
    if (!stringVerify(ISBN)) {
      return res
        .status(400)
        .send({ status: false, message: "ISBN should be of type String." });
    }
    if (!validISBN(ISBN)) {
      return res
        .status(400)
        .send({ status: false, message: "ISBN should be in correct format." });
    }
    ...
    if (checkISBN) {
      return res
        ...
        .send({
          status: false,
          message: "ISBN already used, Please provide another ISBN.",
        });
    }
    ...
    if (!stringVerify(category)) {
      return res
        .status(400)
        .send({ status: false, message: "Category should be of type String." });
    }
    if (!validValue(category)) {
      return res
        .status(400)
        .send({ status: false, message: "Category should be in alphabets." });
    }
    ...
    if (!stringVerify(subcategory)) {
      return res
        ...
        .send({
          status: false,
          message: "Subcategory should be of type String.",
        });
    }
    if (!validValue(subcategory)) {
      return res
        .status(400)
        .send({
          status: false,
          message: "Subcategory should be in alphabets.",
        });
    }
    ...
    if (!stringVerify(releasedAt)) {
      return res
        .status(400)
        .send({
          status: false,
          message: "ReleaseAt should be of type String.",
        });
    }
    if (!validDate(releasedAt)) {
      return res
        .status(400)
        .send({ status: false, message: "Please enter a valid date format." });
    }
  ...
  try {
    ...
    if (Object.keys(req.query).length > 0) {
      if (req.query.userId === "") {
        return res
          .status(400)
          .send({ status: false, message: "Enter value in userId." });
      }
      if (userId) {
      ...
      if (req.query.category === "") {
        return res
          .status(400)
          .send({ status: false, message: "Enter value in category." });
      }

      if (req.query.subcategory === "") {
        return res
          .status(400)
          .send({ status: false, message: "Enter value in subcategory." });
      }

      let getbooks = await bookModel
        .find({ $and: [req.query, { isDeleted: false }] })
        .select({
          _id: 1,
          title: 1,
          excerpt: 1,
...
const getBookById = async function (req, res) {
  try {
    ...
    if (!bookData)
      return res
        .status(404)
        .send({ status: false, message: "No books found with this id." });
...
const updateBookById = async function (req, res) {
  ...
  if(file) data['bookcover']=uploadFile(file)
    ...
    if (title) {
      if (!stringVerify(title)) {
        return res
          ...
          .send({
            status: false,
            message: "Title should be anything but type of String.",
          });
      }
    }
    if (!validTitle(title))
      return res
        ...
        .send({
          status: false,
          message: "Can not have empty request for title",
        });
    ...
    if (excerpt) {
      if (!stringVerify(excerpt)) {
        return res
          ...
          .send({
            status: false,
            message: "Excerpt should be of type String.",
          });
      }
    }
    ...
    if (releasedAt) {
      if (!stringVerify(releasedAt)) {
        return res
          ...
          .send({
            status: false,
            message: "ReleasedAt should be of type String.",
          });
      }
      if (!validDate(releasedAt)) {
        return res
    ...
    if (!validTitle(ISBN))
      return res
        ...
        .send({
          status: false,
          message: "Can not have empty request for ISBN",
        });
...
const deleteBookById = async function (req, res) {
  try {
    ...
    const ddata = await bookModel.findOneAndUpdate(
      { _id: bookId },
